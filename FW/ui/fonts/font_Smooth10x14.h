#ifndef __FONT_SMOOTH_10x14_H__
#define __FONT_SMOOTH_10x14_H__

#include "font.h"

class FontSmooth10x14_t : public Antialiased_4_BitmapFont_t
{
  public:
    FontSmooth10x14_t()
    {
      cachedCharacter = 0;
      cache = reinterpret_cast<Color_t *>(malloc(getFontHeight() * getBoxWidth(' ') * sizeof(Color_t)));
	  assert(cache);
    }
    virtual inline uint32_t firstCharacter() {return FIRST_CHAR;};
    virtual inline uint32_t lastCharacter() {return LAST_CHAR;};
    virtual inline uint32_t getFontHeight() {return FONT_HEIGHT;};
    virtual inline uint32_t getCharWidth(uint32_t chr) {(void)chr; return CHAR_WIDTH;};
    virtual inline uint32_t getBoxWidth(uint32_t chr) {(void)chr; return BOX_WIDTH;};
    virtual inline bool hasGlyph(uint32_t chr) {return ((chr >= FIRST_CHAR) && (chr <= LAST_CHAR));};
  protected:
    static const uint32_t FONT_HEIGHT   = 14;
    static const uint32_t CHAR_WIDTH    = 10;
    static const uint32_t BOX_WIDTH     = 10;
    static const uint32_t FIRST_CHAR    = 32;
    static const uint32_t LAST_CHAR     = 128;
    static const uint32_t DOTS_PER_BYTE = 4;
    static constexpr uint8_t bytes[((LAST_CHAR - FIRST_CHAR + 1) * CHAR_WIDTH * FONT_HEIGHT / DOTS_PER_BYTE)] =
    {
    ...
    virtual inline uint8_t getByte(uint32_t index)
    {
      return bytes[index];
    }
    
    virtual inline uint32_t getBitsPerPixel()
    {
      return 8 / DOTS_PER_BYTE;
    }
};

#endif /* __FONT_SMOOTH_10x14_H__ */