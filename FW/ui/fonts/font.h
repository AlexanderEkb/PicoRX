...
class Antialiased_4_BitmapFont_t : public Font_t
{
  ...
  protected:
    ...
    void updateCache(uint32_t chr)
    {
      ...
      uint32_t index = (chr - firstCharacter()) * characterWidth * fontHeight / DOTS_PER_BYTE;

      uint32_t rem = 0;
      Color_t buffer[DOTS_PER_BYTE];
      
      for (uint32_t col=1; col<characterWidth; col++)
      {
        Color_t * dest = &(cache[boxWidth - col]);
        for (uint32_t row=0; row<fontHeight; row++)
        {
          if (rem == 0)
          {
